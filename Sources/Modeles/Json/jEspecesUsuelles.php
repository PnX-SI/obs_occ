        ...
        case 'genre':
            if ($_REQUEST['filtre'] == 'Habitat') {
                $req = "SELECT cd_cb || ' - ' || lb_cb97_fr AS espece FROM INPN.TYPO_CORINE_BIOTOPES
                    WHERE lb_cb97_fr IS NOT NULL AND cd_cb LIKE '" . mb_substr($critere, 0, 1, 'UTF-8') . "%' ORDER BY espece";
            }
            else {
                $req = "SELECT DISTINCT(split_part(nom_vern, ' ', 1)) AS espece FROM
                    INPN.TAXREF WHERE regne = '" . $_REQUEST['filtre'] . "' AND
                    UPPER(split_part(nom_vern, ' ', 1)) LIKE UPPER('" .
                    mb_substr($critere, 0, 1, 'UTF-8') . "%') ORDER BY espece";
            }
            break;
        case 'espece':
            if ($_REQUEST['filtre'] == 'Habitat') {
                ...
                $req = "SELECT DISTINCT(nom_vern) AS espece FROM INPN.TAXREF WHERE
                    regne = '" . $_REQUEST['filtre'] . "' AND UPPER(split_part(nom_vern, ' ', 1))
                    = UPPER('" . $critere . "') ORDER BY espece";
            }
            break;
    }
